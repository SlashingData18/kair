...
def is_filepath(x):
    # Accept strings or Path-like objects without importing pathlib explicitly
    return is_str(x) or isinstance(x, os.PathLike)
...
def fopen(filepath, *args, **kwargs):
    if is_str(filepath):
        ...
        # ensure PathLike objects are opened via string path
...
def scandir(dir_path, suffix=None, recursive=False, case_sensitive=True):
    ...
    """
    if isinstance(dir_path, (str, os.PathLike)):
